...
class Studente(Utente):
    ...
    def get_corsi_attivi(self):
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            for r in cur.fetchall():
                corsi.append(
                    {"nome_materia": r[0], "professore": f"Prof. {r[1]}" if r[1] else "Nessun Prof", "anno": r[2],
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            query = """
                    SELECT m.ID_MATERIA, \
                           ...
                           (SELECT MIN(Data) \
                            FROM Appello a \
                            WHERE a.COD_MATERIA = m.ID_MATERIA AND a.Data >= date ('now')) as Prossimo_Appello
                    FROM Materia m
            ...
            for r in cur.fetchall():
                materie.append({"id_materia": r[0], "nome_materia": r[1], "anno": r[2], "cfu": r[3],
    ...
    def iscrivi_materia(self, id_materia):
        try:
            with sqlite3.connect(DB_PATH) as conn:
                cur = conn.cursor()
                cur.execute("INSERT INTO Materia_Studenti (COD_STUDENTE, COD_MATERIA) VALUES (?, ?)",
                            (self.id_utente, id_materia))
                conn.commit()
            return True
    ...
    def get_appelli(self):
        ...
        # Qui il try/except è gestito in uscita verso la Dashboard!
        with sqlite3.connect(DB_PATH) as conn:
            cur = conn.cursor()
            # La subquery "COUNT(*) < a.Limite_Iscritti" garantisce che gli appelli pieni non vengano nemmeno mostrati.
            query_disp = """
                         ...
                         WHERE ms.COD_STUDENTE = ?
                           ...
                           AND (SELECT COUNT (*) FROM Prenotazione p WHERE p.COD_APPELLO = a.ID_APPELLO) \
                             < a.Limite_Iscritti \
                         """
    ...
    def prenota_appello(self, id_appello):
        try:
            with sqlite3.connect(DB_PATH) as conn:
                ...
                # --- CONTROLLO FINALE LATO SERVER ---
                # Evita che due studenti clicchino "Prenota" contemporaneamente superando il limite
                cur.execute("SELECT Limite_Iscritti FROM Appello WHERE ID_APPELLO = ?", (id_appello,))
                risultato_limite = cur.fetchone()
                if not risultato_limite:
                    return False
                limite = risultato_limite[0]

                cur.execute("SELECT COUNT(*) FROM Prenotazione WHERE COD_APPELLO = ?", (id_appello,))
                iscritti_attuali = cur.fetchone()[0]

                if iscritti_attuali >= limite:
                    return False  # Capacità esaurita proprio in questo istante
                # ------------------------------------

                cur.execute("INSERT INTO Prenotazione (COD_STUDENTE, COD_APPELLO) VALUES (?, ?)",
                            (self.id_utente, id_appello))
                conn.commit()
            return True
        except sqlite3.Error:
            return False
    ...
    def get_dati_libretto(self):
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            for r in cur.fetchall():
                superati.append(
                    {"materia": r[0], "voto": r[1], "cfu": r[2], "gruppo": "SI" if r[3] else "NO", "id_libretto": r[4]})
        ...
        try:
            with sqlite3.connect(DB_PATH) as conn:
                ...
                if accetta:
                    ...
                    dati = cur.fetchone()
                    if dati:
                        cur.execute("INSERT INTO Libretto (Voto, COD_STUDENTE, COD_APPELLO) VALUES (?, ?, ?)",
